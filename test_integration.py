...
<rss version="2.0" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd">
  <channel>
    ...
    <item>
      ...
      <description>Discussion about the latest technology trends shaping 2024</description>
      <author>tech@example.com (Tech Editor)</author>
      <guid isPermaLink="false">tech-trends-2024</guid>
      <pubDate>Mon, 01 Jan 2024 10:00:00 GMT</pubDate>
      <enclosure url="https://example.com/audio/tech-trends-2024.mp3" length="25000000" type="audio/mpeg"/>
      <itunes:author>Tech Editor</itunes:author>
      <itunes:summary>Discussion about the latest technology trends shaping 2024</itunes:summary>
    ...
    <item>
      ...
      <description>In-depth analysis of voter preferences and political trends</description>
      ...
      <enclosure url="https://example.com/audio/election-analysis-2024.mp3" length="30000000" type="audio/mpeg"/>
      <itunes:author>Political Correspondent</itunes:author>
      <itunes:summary>In-depth analysis of voter preferences and political trends</itunes:summary>
    ...
    <item>
      ...
      <description>Sponsored content about premium creator tools and services</description>
      ...
      <enclosure url="https://example.com/audio/premium-tools-ad.mp3" length="5000000" type="audio/mpeg"/>
      <itunes:author>Advertising Team</itunes:author>
      <itunes:summary>Sponsored content about premium creator tools and services</itunes:summary>
    ...
    <item>
      ...
      <description>Exploring the latest AI and machine learning breakthroughs and their tech impact</description>
      ...
      <enclosure url="https://example.com/audio/ai-ml-breakthroughs.mp3" length="28000000" type="audio/mpeg"/>
      <itunes:author>AI Expert</itunes:author>
      <itunes:summary>Exploring the latest AI and machine learning breakthroughs and their tech impact</itunes:summary>
...
def test_process_feed_appending():
    """Test that process_feed correctly appends new episodes without duplication."""
    ...
    try:
        # Step 2: Run process_feed once to create initial file with tech episodes
        ...
        assert len(initial_feed.entries) == 1, "Should have 1 tech episode initially"
        assert initial_feed.entries[0].title == "Latest Tech Trends 2024"
        
        print("Initial file created successfully with tech episode")
        
        # Step 3: Modify the monkeypatched feed by creating a new version with additional episode
        ...
        def mock_parse_with_new_episode(url_or_file, *args, **kwargs):
            ...
            test_url_mapping = {
                'http://test/feed1': str(modified_feed_path),  # Point to modified feed
                'http://test/feed2': 'minimal_feed.xml', 
                'http://test/feed3': 'empty_feed.xml',
            }
            ...
            if isinstance(url_or_file, str) and url_or_file in test_url_mapping:
                ...
                if xml_file_path.startswith('/'):
                    ...
                    return original_parse(str(test_data_dir / xml_file_path), *args, **kwargs)
        ...
        config2 = FeedConfig(
            url='http://test/feed1',  # Same URL but now points to modified feed
            output=str(output_path),
        ...
        assert len(final_feed.entries) == 2, f"Should have 2 episodes, got {len(final_feed.entries)}"
        
        # Verify no duplication - all episodes should have unique IDs
        episode_ids = [entry.get('id') or entry.get('link') for entry in final_feed.entries]
        assert len(set(episode_ids)) == len(episode_ids), "All episodes should have unique IDs"
        
        # Verify both episodes are present
        titles = [entry.title for entry in final_feed.entries]
        assert "Latest Tech Trends 2024" in titles, "Original tech episode should be preserved"
        assert "New Episode: AI and Machine Learning Breakthroughs" in titles, "New AI episode should be added"
        ...
        # However, feedgen reverses the order, so newest entries appear first
        print("Episode order verified - new episodes added after existing ones as implemented")
